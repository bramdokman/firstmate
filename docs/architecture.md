...
When every idle-baseline sample misses a fast complete turn, herdr falls back to a bounded pane postcondition tied to a pre-send capture, and returns its non-secret proof categories to `fm-send.sh` through a private temporary sidecar so the exact verdict vocabulary stays unchanged; the boundary is in [Current transport behavior](herdr-backend.md#current-transport-behavior).
...
After that proof, the project-declared [task bootstrap](configuration.md#project-task-bootstrap-firstmatebootstrap) verifies or prepares preserved dependency and generated state before any worker launches.
