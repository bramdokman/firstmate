...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, merge-monitored lanes that remain unmerged past that same window, and heartbeat backstop hits.
One narrow exemption sits inside that stale set: a lane whose own status stream folds to a lane-level terminal `done:` with no captain decision still open, and whose exact recorded PR carries a fully authenticated armed merge poll, is monitored through that poll rather than through its deliberately idle pane, so it is absorbed instead of wedge-escalated.
The exemption never applies to keyed sub-activity completions, unarmed or invalid polls, lanes that never reached terminal done, or lanes that resumed `working:`, and it is bounded: because the poll speaks only on a successful merge, the lane is re-surfaced once per `FM_PAUSE_RESURFACE_SECS` window so a PR closed unmerged or stuck in a merge queue cannot rot invisibly.
...
A lane absorbed as merge-monitored makes the same trade on the same window, and both rechecks are anchored on the lane's own status file rather than on its pane hash, so a redrawing idle pane can neither reset nor escape the cadence.
