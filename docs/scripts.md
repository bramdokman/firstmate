...
| `fm-fleet-reconcile.sh`  | Report only disagreements, and checks it could not complete, between durable fleet claims and read-only GitHub observations, holding current draft changes out of vacuous-green scope because a draft asserts no readiness to disagree with |
